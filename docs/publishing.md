...
### Agent Runbook: kash Specifics

Repo-specific additions to the checklist above (keep this section when updating the
template).

1. **Release notes**: load `tbd guidelines release-notes-guidelines` and write the
   aggregate delta since the last tag (not a commit recap), using the format below.
   If the release changes dependency policy (pins, overrides, floors), say what
   *installed environments* actually get: `tool.uv` overrides and constraints are
   lock-only and do NOT ship in wheel metadata.

2. **If `gh` is unavailable** (some sandboxes block it or the GitHub API), the
   equivalent flow is:

   ```shell
   # Annotated tag carries the release notes; tag the merge commit on main.
   git tag -a vX.Y.Z <sha> -F release-notes.md
...
3. **Confirm on PyPI, scriptably** (the JSON API updates within ~a minute):
   ...
   ```shell
   curl -s https://pypi.org/pypi/kash-shell/json | python3 -c \
     "import json,sys; print(json.load(sys.stdin)['info']['version'])"
   ```

   Then smoke-test the published artifact: `uvx --from kash-shell==X.Y.Z kash --version`.

4. **Downstream follow-through** (the kash release order is
   kash → kash-docs → kash-media → apps like deep-transcribe):

   - Bump `kash-shell==X.Y.Z` in kash-docs (and kash-docs in kash-media, etc.).
   - A same-day first-party release is inside the 14-day cool-off window; use a
     per-package exception in the downstream repo rather than weakening the global
     policy, and remove it at the next routine upgrade:
   ...
   - File or close the matching tbd beads and run `tbd sync`.
