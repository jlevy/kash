# Fork of rich/markdown.py, which has original copyright:
# Copyright (c) 2020 Will McGugan
# MIT License
# https://github.com/Textualize/rich/blob/v13.9.4/rich/markdown.py
...
# - Single newline padding around code blocks
# - Output should be generally close to Markdown
# - Item bullets are `-` just like input (not a Unicode bullet)
# - Include ``` fences around code blocks
# - Use a period on enumerated lists (`1.`, `2.`, etc.)
# - Turn off super-prominent h1 formatting with a Panel
...
FEATURES = MarkdownFeatures(
    prominent_h1s=False,  # Kind of overkill
    include_fences=True,  # Important to condition LLMs to use these
)
...
class Heading(TextElement):
    ...
    def __rich_console__(self, console: Console, options: ConsoleOptions) -> RenderResult:
        ...
        if self.tag == "h1" and FEATURES.prominent_h1s:
...
class Markdown(JupyterMixin):
    """Customized for kash. A Markdown renderable.
    ...
    def __init__(
        ...
        markup = str(markup)  # Convenience for lazy objects.
        self.markup = markup
...
# XXX Monkey patch our custom Markdown class into Rich's.
def patch_rich_markdown():
