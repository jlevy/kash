...
def input_param_name(prompt_text: str, settable_params: dict[str, Param]) -> Param:
    param_name = input_choice(
        prompt_text,
        choices=list(settable_params.keys()),
        mandatory=True,
    )
    return settable_params[param_name]


def input_param_value(prompt_text: str, param: Param) -> str:
